...
class NoteBookOperator(GKEStartPodOperator):
    ...
    def __init__(self,
                 ...
                 bootstrap_script_url: str = None,*args, **kwargs):
        """
        ...
        if not self.bootstrap_script_url:
            self.bootstrap_script_url = EXPLORER_BOOTSCRIPT_URL

        if not self.requirements_url:
            self.requirements_url = EXPLORER_REQUIREMENTS_URL
        ...
        kwargs['cmds'] = ['sh', '-c']
        kwargs['arguments'] = [''.join(self.arguments)]
        kwargs['in_cluster'] = False
